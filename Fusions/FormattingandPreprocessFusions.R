...
JAFFAL_LR_Huh7 <- read.csv("/bioinformatics/ryley/Library_Benchmark/JAFFAL_Huh7_gv43/JAFFAL_Huh7_genCode43/jaffa_results.csv", header = TRUE) %>% 
  mutate(library_type = dplyr::case_when(grepl('FBA22517|FBA22660|dRNA', sample)  ~ "direct_RNA",
                                  grepl('FBA43334|FBA62655|dcDNA', sample)  ~ "direct_cDNA",
                                  grepl('FAZ|PCR|FLNCcDNA', sample)  ~ "PCR_cDNA"),
         RNA_sample = dplyr::case_when(grepl('FBA62655|FBA22660|FAZ80247|B1', sample)  ~ "B1", 
...
JAFFAL_Illumina_Huh7 <- read.csv("/bioinformatics/ryley/JAFFA_Promethion/jaffa_results.csv", header = TRUE)%>%
  mutate(Algorithm = "JAFFA-direct",
         Platform = "Illumina",
         library_type = "PCR_cDNA",
         Cell_Line = "Huh7") %>%
  mutate(RNA_sample = dplyr::case_when(grepl('B1', sample)  ~ "B1", 
                                grepl('B2', sample) ~ "B2"))
...
for (location in LRTri_gene){
  ...
  assign(paste0("file_", name), 
         mutate(read.table(location, header = TRUE), 
                sample = location, 
                library_type = dplyr::case_when(grepl('FBA22517|FBA22660|dRNA', sample)  ~ "direct_RNA",
                                         grepl('FBA43334|FBA62655|dcDNA', sample)  ~ "direct_cDNA",
                                         grepl('FAZ|PCR|FLNCcDNA', sample)  ~ "PCR_cDNA"),
                RNA_sample = dplyr::case_when(grepl('FBA62655|FBA22660|FAZ80247|B1', sample)  ~ "B1", 
                                       grepl('FBA22517|FAZ83542|FBA43334|B2', sample) ~ "B2"), 
...
Genion_Huh7<- do.call(rbind, lapply(myfiles, function(filename) {
  if (file.info(filename)$size > 0) {
    read.table(filename) %>%
      dplyr::mutate(Source = sub("_genion$", "", basename(filename)))
  } else {
    NULL
  }
}))
Genion_Huh7<- Genion_Huh7 %>% 
  ...
  dplyr::mutate(Algorithm = "Genion",
         ...
         RNA_sample = dplyr::case_when(grepl('FBA62655|FBA22660|FAZ80247|B1', Source)  ~ "B1", 
...
LongGF_Huh7 <- LongGF_Huh7 %>% 
  ...
  dplyr::filter(chromosome1 %in% standard_chrs)%>%
  dplyr::filter(chromosome2 %in% standard_chrs)%>%
  dplyr::filter(V3 >= 2)%>%
  mutate(Algorithm = "LongGF",
         library_type = dplyr::case_when(grepl('FBA22517|FBA22660|dRNA', V1)  ~ "direct_RNA",
                                  grepl('FBA43334|FBA62655|dcDNA', V1)  ~ "direct_cDNA",
                                  grepl('FAZ|PCR|FLNCcDNA', V1)  ~ "PCR_cDNA"),
         RNA_sample = dplyr::case_when(grepl('FBA62655|FBA22660|FAZ80247|B1', V1)  ~ "B1", 

...
Recall_Precision <- ggplot(filter(stat_summary, control == 'positive'))+
  ...
  facet_grid(depth ~ Sequence_Identity)+
  labs(x = "Precision", y = "Recall", Title = "NEW RECALL & PRECISION ANALYSIS")+
  ylim(c(0,1.00))+
  xlim(c(0,1.00)) +
...
Figure3A <-((Precision_subset /  Recall_subset + plot_layout(guides = 'keep')) |  F1_subset)
```
...
Make Figure 1 part A-D
```{r}
Figure1A <- ggplot(filter(combined_data_annotation, control == "positive", depth == "100GB", Sequence_Identity == "95%")) +
...
Figure6B_2 <-ggplot(filter(fusion_profile_stat_summary, control == "positive", recall_category == "False_Call", Sequence_Identity == "95%"),
                    aes(x = depth, y = fusion_type_count, colour = Algorithm, group = interaction(Algorithm, Sequence_Identity)))+ 
  geom_point() +
  geom_line()+
  facet_wrap(~fusionType, labeller = as_labeller(c(c("wrong_order:tri_fusion" = "Wrong Order Tri-fusion", 
...
Figure6_sup <- (Figure6A_1_sup + Figure6A_2_sup)/
  ...
  plot_annotation(theme = theme(plot.title = element_text(size = 14, face = "bold")))
ggsave("Figure6_sup.pdf", plot = Figure6_sup, width = 270 , height = 270 , unit ="mm")
```
...
Figure7_sup <- (Figure7A_1_sup + Figure7A_2_sup)/
  ...
  plot_annotation(theme = theme(plot.title = element_text(size = 14, face = "bold")))
ggsave("Figure7_sup.pdf", plot = Figure7_sup, width = 270 , height = 270 , unit ="mm")
```
...
Figure2 <- (Figure2A_1 + Figure2A_2)/
  ...
  plot_annotation(theme = theme(plot.title = element_text(size = 14, face = "bold")))
ggsave("Figure2_paper.pdf", plot = Figure2, width = 297, height = 210, units = "mm")
```
...
ggsave("Figure3B_paper.pdf", plot = Figure3B, width = 297, height = 210, units = "mm")  
```
Make Compound Figure 3
```{r}
Figure3<- Figure3A + Figure3B
ggsave("Figure3A_paper.pdf", plot = Figure3A, width = 297, height = 210, units = "mm")
ggsave("Figure3B_paper.pdf", plot = Figure3B, width = 297, height = 210, units = "mm")  
...
ggsave("Figure1E.pdf", plot = True_overlap, width = 297, height = 210, units = "mm")
```
Create Compound Figure 1:
```{r}
Figure1_paper <- (Figure1A | Figure1B| Figure1C | Figure1D)/
                  (True_overlap) +   
  plot_annotation(theme = theme(plot.title = element_text(size = 14, face = "bold")))
ggsave("Figure1_paper.pdf", plot = Figure1_paper)
```
...
library(ggforce)
#Figure 4 ####
Figure4 <- ggplot(filter(absolute_Breakpoint_Accuracy, seq_depth == "100GB", seq_id == "95%"), aes(y = values_vector, x = Breakpoint_number, color = Algorithm)) +
  geom_boxplot(outlier.shape = NA) +
...
ggsave("Figure4_paper.pdf", plot = Figure4, width = 297, height = 210, units = "mm")
```

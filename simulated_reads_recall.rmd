...
```{r}
Genion_Fffscores <- Annot_Genion_Sim %>%  mutate(recall_category = case_when(
...
ggplot(filter(Genion_Fffscores, control =="positive", V7=="PASS:RT"))+
  geom_point(aes(x = V4, y= V3.x, colour = recall_category, shape = fusionType))+
  facet_grid(depth ~ Sequence_Identity)+
  scale_x_log10(breaks = 10^(-5:3))+
  labs(x="FiN score", y="ffigf score")+ 
  theme(legend.title = element_blank())
...
JAFFAL_scoring <- Annot_JAFFAL_Sim %>%  mutate(recall_category = case_when(
      grepl("false", fusionType) ~ "False_Call",
      grepl("truncated|reverse|chromosomal_misalignment|wrong", fusionType) ~ "Partial_Recall",
      TRUE ~ "True_Recall"))

ggplot(filter(JAFFAL_scoring, control =="positive"))+
  geom_boxplot(aes(y = spanning.reads, x = classification, fill =recall_category ))+
  facet_nested(depth ~ Sequence_Identity)+
  theme(legend.title = element_blank(), axis.text.x = element_text(angle = 45, vjust = 0.5, hjust=0.5))+
  labs(x = "", y ="supporting reads")

ggplot(filter(JAFFAL_scoring, control =="positive", depth == "100GB", Sequence_Identity == "95%"))+
  geom_boxplot(aes(y = spanning.reads, x = classification, fill =recall_category ))+
  theme(legend.title = element_blank(), axis.text.x = element_text(angle = 45, vjust = 0.5, hjust=0.5))+
  labs(title = "Sequencing Depth 100Gb , 95% Mean Sequence Identity",
       x = "", y ="supporting reads")

JAF_RT <- ggplot(filter(JAFFAL_scoring, control =="positive", depth == "100GB", Sequence_Identity == "95%", fusionType=="read_through"))+
  geom_boxplot(aes(y = spanning.reads, x = classification, fill = fusionType))+
  theme(legend.title = element_blank(), axis.text.x = element_text(angle = 45, vjust = 0.5, hjust=0.5))+
  labs(title = "Sequencing Depth 100Gb , 95% Mean Sequence Identity",
       subtitle = "JAFFAL's classication of simulated read-throughs", x="")

JAF_PRT <- ggplot(filter(JAFFAL_scoring, control =="positive", depth == "100GB", Sequence_Identity == "95%", classification=="PotentialReadThrough"))+
  geom_boxplot(aes(y = spanning.reads, x = classification, fill = fusionType  ))+
  theme(legend.title = element_blank(), axis.text.x = element_text(angle = 45, vjust = 0.5, hjust=0.5))+
  labs(subtitle = "JAFFAL's Potential Readthrough", x="", y="")+
  ylim(c(0,50))
...
write_tsv(combined_data,"/bioinformatics/ryley/Algorithm_Benchmark/benchmark_data_analysis/Analysis/benchmark_fusion_results/combined_data_FusionCalls.tsv", row.names = FALSE)
```
...
write_tsv(stat_summary, file="/bioinformatics/ryley/Algorithm_Benchmark/benchmark_data_analysis/Analysis/benchmark_fusion_results/stat_summary.tsv")
...
Recall_subset<- ggplot(filter(readsupp_filtering, control == "positive", depth == "100GB", Sequence_Identity == "95%"))+
  ...
  ylim(c(0,1.00))+
  theme(legend.position = "none")
((Precision_subset /  Recall_subset + plot_layout(guides = 'keep')) |  F1_subset)+ plot_layout(guides = 'collect')
```
...
```{r}
Points_profile_data <- fusion_profile_stat_summary
Points_profile_data$depth <- as.numeric(factor(Points_profile_data$depth, levels = c("1GB", "10GB", "100GB")))
False_Recall_Point <- ggplot(filter(Points_profile_data,
            recall_category == "False_Call"), aes( x = depth, y = Sequence_Identity, 
                   colour = Algorithm, size = fusion_type_count))+
  geom_point(position = position_dodge(width = 0.5))+
  facet_grid(fusionType~control, labeller = as_labeller(c(c("wrong_order:tri_fusion" = "Wrong Order Tri-Fusion", 
                             "false_fusion:self_misalignment" = "Self-Misalignment",
                             "false_fusion:mitochondrial_genomic" = "Mitochondrial:Genomic",
                             "false_fusion:mitochondrial" = "Mitochondrial",
                             "false_fusion" = "False Chimera"), 
                             c("Genion"="Genion", "JAFFAL" = "JAFFAL", "LongGF" = "LongGF", "FusionSeeker" = "FusionSeeker"),
                             c("positive"="Postive", "negative" = "Negative"))))+
  scale_x_continuous(
    breaks = 1:3,
    labels = c("1GB", "10GB", "100GB"))+
  labs(x = "Sequencing Depth",
       y = "Mean Sequence Identity")+ 
  theme(aspect.ratio = 1, axis.text.x = element_text(angle=45))
 ggsave("False_Fusions.pdf", plot = False_Recall_Point, width = 297, height = 270, units = "mm")
 
Partial_Recall_Point <-ggplot(filter(Points_profile_data,
            recall_category == "Partial_Recall",
            control == "positive"), aes( x = depth, y = Sequence_Identity, 
                   colour = Algorithm, size = fusion_type_count))+
  geom_point(position = position_dodge(width = 0.5))+
  facet_wrap(~fusionType, ncol = 2, labeller = as_labeller(c(c("reverse_order: hybrid"="(R) Inter-Chromosomal", 
                             "reverse_order: intra_chromosome"="(R) Intra-Chromosomal",
                             "reverse_order: read_through"="(R) Read-through",
                             "wrong_order:tri_fusion"="(W) Tri-Fusion",
                             "reverse_order:tri_fusion"="(R) Tri-Fusion",
  ...
  labs(x = "Sequencing Depth",
       y = "Mean Sequence Identity")+ 
  theme(aspect.ratio = 1, axis.text.x = element_text(angle=45))
  ggsave("Partial_Recall.pdf", plot = Partial_Recall_Point, width = 297, height = 270, units = "mm")
  
True_Recall_Point <- ggplot(filter(Points_profile_data,
            recall_category == "True_Recall",
            control == "positive"), aes( x = depth, y = Sequence_Identity, 
                   colour = Algorithm, size = fusion_type_count))+
  geom_point(position = position_dodge(width = 0.5))+
  ...
  labs(x = "Sequencing Depth",
       y = "Mean Sequence Identity")+ 
  theme(aspect.ratio = 1, axis.text.x = element_text(angle=45))
ggsave("True_Recall.pdf", plot = True_Recall_Point, width = 297, height = 210, units = "mm")
```
...
Figure1D_2 <-ggplot(filter(fusion_profile_stat_summary, control == "positive", recall_category == "True_Recall", depth == "100GB", Sequence_Identity == "95%"))+ 
...
library(patchwork)
Figure1D <- (Figure1D_1 + Figure1D_2)+
  plot_annotation(theme = theme(plot.title = element_text(size = 14, face = "bold")))
ggsave("Figure1D.pdf", plot = Figure1D)
...
Figure1E_2 <-ggplot(filter(fusion_profile_stat_summary, control == "positive", recall_category == "False_Call", depth == "100GB", Sequence_Identity == "95%"))+ 
  ...
  labs(title = "",
       ...
       x = "")
library(patchwork)
Figure1E <- (Figure1E_1 + Figure1E_2)+
  plot_annotation(theme = theme(plot.title = element_text(size = 14, face = "bold")))
ggsave("Figure1E.pdf", plot = Figure1E)
...
Figure1F_2 <-ggplot(filter(fusion_profile_stat_summary, control == "positive", recall_category == "Partial_Recall", depth == "100GB", Sequence_Identity == "95%"))+ 
...
Figure1F <- (Figure1F_1 + Figure1F_2)+
  plot_annotation(theme = theme(plot.title = element_text(size = 14, face = "bold")))
ggsave("Figure1F.pdf", plot = Figure1F, width = 297 , unit ="mm")
```
```{r}
combined_data_annotation$depth <- factor(combined_data_annotation$depth, levels = c("1GB", "10GB", "100GB"))
fusion_profile_stat_summary$depth <- factor(fusion_profile_stat_summary$depth, levels = c("1GB", "10GB", "100GB")) 
...
Figure2A_2 <-ggplot(filter(fusion_profile_stat_summary, control == "positive", recall_category == "True_Recall", Sequence_Identity == "95%"),
...
Figure2A_4 <-ggplot(filter(fusion_profile_stat_summary, control == "positive", recall_category == "False_Call", Sequence_Identity == "95%"),
                    aes(x = depth, y = fusion_type_count, colour = Algorithm, group = interaction(Algorithm, Sequence_Identity)))+ 
  geom_point() +
  geom_line()+
  facet_wrap(~fusionType, labeller = as_labeller(c(c("wrong_order:tri_fusion" = "Wrong Order Tri-fusion", 
...
Figure2A <- (Figure2A_1 + Figure2A_2)/
            (Figure2A_3 + Figure2A_4)+
  plot_annotation(theme = theme(plot.title = element_text(size = 14, face = "bold")))
ggsave("Figure2A.pdf", plot = Figure2A, width = 270 , height = 270 , unit ="mm")
...
Figure2B_2 <-ggplot(filter(fusion_profile_stat_summary, control == "positive", recall_category == "True_Recall", depth == "100GB"),
...
Figure2B_4 <-ggplot(filter(fusion_profile_stat_summary, control == "positive", recall_category == "False_Call", depth == "100GB"),
                    aes(x = Sequence_Identity, y = fusion_type_count, colour = Algorithm, group = interaction(Algorithm, depth)))+ 
  geom_point() +
  geom_line()+
  facet_wrap(~fusionType, labeller = as_labeller(c(c("wrong_order:tri_fusion" = "Wrong Order Tri-fusion", 
...
Figure2B <- (Figure2B_1 + Figure2B_2)/
            (Figure2B_3 + Figure2B_4)+
  plot_annotation(theme = theme(plot.title = element_text(size = 14, face = "bold")))
ggsave("Figure2B.pdf", plot = Figure2B, width = 270 , height = 270 , unit ="mm")
...
fusions_in_n_algorithms <- combined_data %>%
  ...
  mutate(recall_category = case_when(
      ...
      grepl("truncated|reverse|chromosomal_misalignment|wrong", fusionType) ~ "Partial_Recall",
      TRUE ~ "True_Recall"))%>%
  filter(recall_category == "True_Recall") #%>% filter(fusion.gene.id %in% unique(fusions_in_all_algorithms$fusion.gene.id))
...
ReadSupport_Algorithm_True <- ggplot(filter(fusions_in_n_algorithms, depth == "100GB", Sequence_Identity == "95%"), aes(x = fusion.gene.id, y = Algorithm, fill = Total_Read_Supp)) +
  geom_tile() +
  geom_text(aes(label = Total_Read_Supp), color = "black", size = 3, alpha =1) +  # Add numbers to the tiles, adjust color for visibility
  labs(x="", y="",
       fill = "Read Support") +
  theme_minimal() +
  facet_grid(Sequence_Identity ~ depth, scales = "free_x", space = "free")+
    scale_fill_gradient2(low = "lemonchiffon", mid="orange" ,high = "orchid", na.value = "grey", midpoint = 150)+
  scale_alpha_continuous(range = c(0.1, 1)) + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1), strip.background = element_rect(color="black", fill="grey80"))
ggsave("ReadSupport-Algorithm_True.pdf", plot = ReadSupport_Algorithm_True)
...
ReadSupport_Algorithm_All <- ggplot(filter(fusions_in_n_algorithms, depth == "100GB", Sequence_Identity == "95%", control == "positive", overlap == 4), aes(x = fusion.gene.id, y = Algorithm, fill = Total_Read_Supp)) +
  geom_tile() +
  geom_text(aes(label = Total_Read_Supp), color = "black", size = 3, alpha =1) +  # Add numbers to the tiles, adjust color for visibility
  labs(x="", y="",
       fill = "Read Support") +
  ...
  scale_alpha_continuous(range = c(0.1, 1)) + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1), strip.background = element_rect(color="black", fill="grey80"))
ggsave("ReadSupport-Algorithm_All.pdf", plot = ReadSupport_Algorithm_All)

ggplot(filter(fusions_in_n_algorithms, depth == "100GB", control == "positive", overlap == 4), aes(x = fusion.gene.id, y = Algorithm, fill = Total_Read_Supp)) +
  geom_tile() +
  geom_text(aes(label = Total_Read_Supp), color = "black", size = 3, alpha =1) +  # Add numbers to the tiles, adjust color for visibility
  labs(x="", y="",
       fill = "Read Support") +
...
ggplot(filter(fusions_in_n_algorithms, depth == "100GB", Sequence_Identity == "95%", control == "positive", overlap == 4), aes(x = fusion.gene.id, y = Algorithm, fill = Total_Read_Supp)) +
  geom_tile() +
  geom_text(aes(label = Total_Read_Supp), color = "black", size = 3, alpha =1) +  # Add numbers to the tiles, adjust color for visibility
  labs(x="", y="",
       fill = "Read Support") +
...
ggplot(filter(fusions_in_n_algorithms, depth == "100GB", Sequence_Identity == "95%", control == "positive", overlap >= 2, recall_category == "True_Recall"), aes(x = fusion.gene.id, y = Algorithm, fill = Total_Read_Supp)) +
  geom_tile() +
  geom_text(aes(label = Total_Read_Supp), color = "black", size = 3, alpha =1) +  # Add numbers to the tiles, adjust color for visibility
  labs(x="", y="",
       fill = "Read Support") +
...
ggplot(filter(fusions_in_n_algorithms, depth == "100GB", control == "positive", overlap >= 2, recall_category == "True_Recall"), aes(x = fusion.gene.id, y = Algorithm, fill = Total_Read_Supp)) +
  geom_tile() +
  geom_text(aes(label = Total_Read_Supp), color = "black", size = 3, alpha =1) +  # Add numbers to the tiles, adjust color for visibility
  labs(x="", y="",
       fill = "Read Support") +
  ...
  facet_nested(Sequence_Identity ~ depth + recall_category , scales = "free_x", space = "free")+
    scale_fill_gradient2(low = "lemonchiffon", mid="orange" ,high = "orchid", na.value = "grey", midpoint = 50)+
  scale_alpha_continuous(range = c(0.1, 1)) + 
  theme(axis.text.x = element_text(angle = 60, hjust = 1), strip.background = element_rect(color="black", fill="grey80"))

ggplot(filter(fusions_in_n_algorithms, control == "positive", Sequence_Identity == "95%", overlap >= 2, recall_category == "True_Recall"), aes(x = fusion.gene.id, y = Algorithm, fill = Total_Read_Supp)) +
  geom_tile() +
  geom_text(aes(label = Total_Read_Supp), color = "black", size = 3, alpha =1) +  # Add numbers to the tiles, adjust color for visibility
  labs(x="", y="",
       fill = "Read Support") +
  theme_minimal() +
  facet_nested(depth ~ Sequence_Identity  + recall_category , scales = "free_x", space = "free")+
    scale_fill_gradient2(low = "lemonchiffon", mid="orange" ,high = "orchid", na.value = "grey", midpoint = 50)+
  scale_alpha_continuous(range = c(0.1, 1)) + 
  theme(axis.text.x = element_text(angle = 60, hjust = 1), strip.background = element_rect(color="black", fill="grey80"))
```
...
```{r}
zhi_fusion_summary <- read.table("~/LongReadFusionCallerBenchmark/fusion_summary.tsv", header = TRUE, sep = "\t")
zhi_fusion_with_exonSeq <- read.table("~/LongReadFusionCallerBenchmark/fusion_with_exonSeq.tsv", header = TRUE, sep = "\t")
...
```{r}
library(ggforce)
ggplot(Breakpoint_Accuracy, aes(x = values_vector, y = Breakpoint_number, color = Algorithm)) +
  geom_violin() +
  geom_sina()+
  theme_minimal()+ 
  scale_x_log10(breaks = 10^(0:7), labels = label_log(digits = 2))+
  labs(x = "Absolute distance from simulated breakpoint") 
  
absolute_Breakpoint_Accuracy <- Breakpoint_Accuracy %>%
  mutate(values_vector = abs(values_vector))

with0_breakpoint_distance_graph <- ggplot(filter(absolute_Breakpoint_Accuracy, seq_depth == "100GB", seq_id == "95%"), aes(y = values_vector, x = Breakpoint_number, color = Algorithm)) +
  geom_violin() +
  geom_sina()+
  theme_minimal()+ 
  ...
  labs(y = "Absolute distance from simulated breakpoint",
       x= "")
  ggsave("with0_breakpoint_distance_graph.png", plot = with0_breakpoint_distance_graph)
with0_breakpoint_distance_graph

facet_with0_breakpoint_distance_graph <- ggplot(filter(absolute_Breakpoint_Accuracy), aes(y = values_vector, x = Breakpoint_number, color = Algorithm)) +
  geom_violin() +
  geom_sina()+
  theme_minimal()+ 
  scale_y_continuous(trans='log1p', labels = label_log(digits = 2),
  ...
  ggsave("facet_with0_breakpoint_distance_graph.png", plot = facet_with0_breakpoint_distance_graph)
  facet_with0_breakpoint_distance_graph
  
  ggplot(filter(absolute_Breakpoint_Accuracy, seq_depth == "100GB", seq_id == "95%"), aes(y = values_vector, x = Breakpoint_number, color = Algorithm)) +
  geom_boxplot()+
  theme_minimal()+ 
  scale_y_continuous(trans='log1p', labels = label_log(digits = 2),
    breaks = c(0, 1, 10, 100, 1000, 10000, 100000, 1000000, 1000000, 10000000, 100000000))+
  ...
  ggplot(filter(absolute_Breakpoint_Accuracy, seq_depth == "100GB", seq_id == "95%"), aes(x = values_vector, color = Algorithm)) +
  geom_density()+
  theme_minimal()+
    scale_x_continuous(trans='log1p', labels = label_log(digits = 2),
    breaks = c(0, 1, 10, 100, 1000, 10000, 100000, 1000000, 1000000, 10000000, 100000000))+
  labs(x = "Absolute distance from simulated breakpoint")+
    facet_col(~Breakpoint_number)
```

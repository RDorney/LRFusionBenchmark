...
p_k562 <- ggplot(filter(counts_SGNex_summary, Sequencing_Depth != "Total", Cell_Lines == "K562")) +
...
p_mcf7 <- ggplot(filter(counts_SGNex_summary, Sequencing_Depth != "Total", Cell_Lines == "MCF7")) +
  geom_point(aes(x = novelnumber, y = knownnumber, colour = Algorithm, size = Sequencing_Depth)) +
  facet_grid(Library ~ Cell_Lines) +
...
SGNex_novelvsknown <- ggplot(filter(counts_SGNex_summary, Sequencing_Depth != "Total"))+
  geom_point(aes(x = novelnumber , y =knownnumber, colour = Algorithm, size = Sequencing_Depth, ))+
...
ggplot(filter(super_SGNex_summary, Sequencing_Depth == "Total"), 
       aes(x = Sequencing_Depth, y = knownnumber, fill = Algorithm)) +
  geom_col() +
